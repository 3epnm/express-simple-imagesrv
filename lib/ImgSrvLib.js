...
const execImage = async (req, res, params, cid, mimeType, json) => {
    const src_path = req.app.get('imgsrv_options').upload_dir + req.params.guid;    
    const href = req.app.get('imgsrv_options').base_url + req.params.guid;  
    const cache_dir = req.app.get('imgsrv_options').cache_dir;
    const logger = req.app.get('imgsrv_options').logger;
...
const getImage = async (req, res, next) => {
    let config = new ConfigMgr();
    console.log(config.getConfig('upload_dir'));
    

    const path = req.app.get('imgsrv_options').upload_dir + req.params.guid;
    const cache_dir = req.app.get('imgsrv_options').cache_dir;
    const logger = req.app.get('imgsrv_options').logger;
    ...
    try {
        ...
        params.set('top', req.query.top && parseInt(req.query.top));
        params.set('left', req.query.left && parseInt(req.query.left));
        params.set('width', req.query.width && parseInt(req.query.width));
        params.set('height', req.query.height && parseInt(req.query.height));
        params.set('grayscale', req.query.toGrayscale);
        ...
        let uid = uuidv4(), cacheMgr = req.app.get('imgsrv_options').cache_mgr;
        cacheMgr
            .acquire(cid, uid)
            .then(async (msg) => {
...
const postImageUpload = async (req, res, next) => {
    const path = req.app.get('imgsrv_options').upload_dir + req.file.filename;
    const href = req.app.get('imgsrv_options').base_url + req.file.filename;
    const logger = req.app.get('imgsrv_options').logger;
...
const postImageJson = async (req, res, next) => {
    const href = req.app.get('imgsrv_options').base_url;
    const logger = req.app.get('imgsrv_options').logger;
    ...
    return new Promise((result, reject) => {
        crypto.pseudoRandomBytes(16, async function (err, raw) {
            ...
            try {
                ...
                const path = req.app.get('imgsrv_options').upload_dir + uid;
